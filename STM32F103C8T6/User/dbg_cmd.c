...
struct dbg_cmd_s {
...
#endif
#if ( PARAM_DOUBLE_NUM >= 1 )
    double           param_double_buf[PARAM_DOUBLE_NUM];
#endif
#if ( PARAM_SCI_NUM >= 1 )
    double           param_sci_buf[PARAM_SCI_NUM];
#endif
    ...
    unsigned char    param_double_cnt;
    unsigned char    param_sci_cnt;
...
static char search_replace_char( char *str, char fchar, char bchar, char len)
{
    char i = 0;
...
static char cmd_end_is_enter(char *str)
{
    if (*str == '\r') {
        return true;
    } else {
...
static char param_type_1byte(char *str, char *val) /*参数为1字节类型*/
{
    char result = true;
    char i = 0, temp8 = 0;
...
static char param_type_2byte(char *str, short *val) /*参数为2字节类型*/
{
    char result = true;
    char i = 0;
    short temp16 = 0;
...
#if ( PARAM_4BYTE_NUM >= 1 )
static char param_type_4byte(char *str, long *val) /*参数为4字节类型*/
{
    char result = true;
    char  i = 0;
    long tempu32 = 0;
...
#if ( PARAM_FOLAT_NUM >= 1 )
static char param_type_float(char *str, float *val) /*参数为浮点类型*/
{
    char result = true;
...
#if ( PARAM_DOUBLE_NUM >= 1 )
static char param_type_double(char *str, double *val) /*参数为浮点类型*/
{
    char result = true;
    char i = 0, flag = 0, sign = 0;
    double integer = 0, decimal = 0, tempf = 0,decimal_10enx = 0;
    if (*str == '-') { //负号
        str++;
        dbg_cmd.cmd_str_pos++;
        sign = 1;
...
#if ( PARAM_SCI_NUM >= 1 )
static char param_type_sci(char *str, double *val) /*参数为浮点类型*/
{
    char result = true;
    char i = 0, flag = 0, sign = 0, e_sign = 0, sci_e = 0;
    double integer = 0, decimal = 0, tempf = 0,decimal_10enx = 0;
    if (*str == '-') { //负号
        str++;
        dbg_cmd.cmd_str_pos++;
...
#if ( PARAM_STRING_NUM >= 1 )
static char param_type_str(char *str, char Array[]) /*参数为字符串*/
{
    char result = true;
    char i = 0,mode = 0;
    if (*str == '"') {
...
// Param Analy
static char param_type_analy(char param_type)
{
    switch (param_type) {
        case '1':
...
#if ( PARAM_DOUBLE_NUM >= 1 )
        case 'd':
            if (dbg_cmd.param_double_cnt >= PARAM_DOUBLE_NUM) {
                return false;
            }
            if (param_type_double(dbg_cmd.rxd_cmd_buf + dbg_cmd.cmd_str_pos, dbg_cmd.param_double_buf + dbg_cmd.param_double_cnt) == false) {
                return false;
            }
            dbg_cmd.param_double_cnt++;
            break;
#endif
#if ( PARAM_SCI_NUM >= 1 )
        case 'e':
            if (dbg_cmd.param_sci_cnt >= PARAM_SCI_NUM) {
                return false;
            }
            if (param_type_sci(dbg_cmd.rxd_cmd_buf + dbg_cmd.cmd_str_pos, dbg_cmd.param_sci_buf + dbg_cmd.param_sci_cnt) == false) {
                return false;
            }
            dbg_cmd.param_sci_cnt++;
            break;
#endif
#if ( PARAM_STRING_NUM >= 1 )
        case 's':
...
typedef char (*ComDbgFunc)(void);
static char dbg_cmd_analy(void)
{
...
static void module_msg()
{
    ...
    DBG_CMD_PRN("ParamD:%d\r\n", PARAM_DOUBLE_NUM);
    DBG_CMD_PRN("ParamE:%d\r\n", PARAM_SCI_NUM);
    DBG_CMD_PRN("InPut 0X3A => h3a or H3a\r\n");
}

static char dbg_cmd_func()
{
    ...
    if (dbg_cmd_exec(CMD_NAME"Time", "111", "<H> <M> <S>")) {
...
#if ( PARAM_DOUBLE_NUM >= 1 )
double get_param_double(char index)
{
    return dbg_cmd.param_double_buf[index];
}
#endif

#if ( PARAM_SCI_NUM >= 1 )
float get_param_sci(char index)
{
    return dbg_cmd.param_sci_buf[index];
}
#endif
...
char dbg_cmd_exec(char *cmd, char param_table[], char help_msg[])
{
    ...
    dbg_cmd.param_double_cnt = 0;
    dbg_cmd.param_sci_cnt = 0;
